    ...
    override suspend fun doWork(): Result {
        ...
        return withContext(Dispatchers.IO) {
            ...
            return@withContext try {
                ...
                // Dùng MediaStore API mới thay vì insertImage (deprecated)
                val imageUrl: String? = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
                    val contentValues = ContentValues().apply {
                        put(MediaStore.Images.Media.DISPLAY_NAME, "$title - ${dateFormatter.format(Date())}.png")
                        put(MediaStore.Images.Media.MIME_TYPE, "image/png")
                        put(MediaStore.Images.Media.RELATIVE_PATH, "Pictures/Bluromatic")
                    }
