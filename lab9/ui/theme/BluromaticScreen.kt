...
fun BluromaticScreen(
    ...
    val imageUri by blurViewModel.imageUri.collectAsStateWithLifecycle()
...
fun BluromaticScreenContent(
    ...
    Column(modifier = modifier) {
        ...
        // Hiển thị ảnh: ưu tiên outputUri khi đã blur xong, sau đó ảnh gốc, cuối cùng cupcake
        val displayUri = when (blurUiState) {
            is BlurUiState.Complete -> blurUiState.outputUri  // ảnh blur
            else -> currentImageUri?.toString() ?: ""          // ảnh gốc hoặc rỗng
        }

        val painter = if (displayUri.isNotEmpty()) {
            rememberAsyncImagePainter(model = Uri.parse(displayUri))
        } else {
    ...
    Row(
        ...
        when (blurUiState) {
            ...
            is BlurUiState.Complete -> {
                // Hiển thị nút Start lại để blur tiếp nếu muốn
                Button(
                    onClick = onStartClick,
                    modifier = Modifier.fillMaxWidth()
                ) {
